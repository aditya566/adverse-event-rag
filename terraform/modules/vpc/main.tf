...
# Private subnets: RDS, Lambda, ECS tasks
...
# ── NAT Gateway (allows private subnets to reach internet for Bedrock/S3) ────
...
# ── VPC Endpoints (keeps S3/Bedrock traffic inside AWS network) ───────────────
...
resource "aws_vpc_endpoint" "bedrock" {
  ...
  security_group_ids  = [aws_security_group.vpc_endpoints.id]
  private_dns_enabled = true
  tags                = { Name = "${var.name}-bedrock-endpoint" }
}
